...
The earlier dose--response analysis was repeated using the surrogate-gain quantities, resolved by block and checkpoint. Only pre-collapse checkpoints are included. The trajectory panel follows the highest-gain block of each run over training, while two dose--response panels compare the pre-onset burst rate with the worst-block gain measured at a common reference step 2000, thousands of steps before the earliest observed collapse, and at initialization (step 0, before any gradient update). The initialization values are computed from a single untrained forward pass: at fixed seed the initial weights are identical across configurations, so only \((K,J,T)\) vary. The step-0 dose--response shows that the initialization value of the surrogate gain already separates the configurations that later collapse (all observed collapses have worst-block \(\Pisurr(0)\gtrsim250\)); this provides a configuration-screening rule that requires no training, although, being configuration-level, it carries no within-run temporal information.
...
The dataset includes eighteen fixed-\(T\) campaign runs with checkpointed score ladders for \(K=32,64\) (20k-step horizon), the sixteen-configuration boundary-bracketing campaign over \(K=32\)--128 (10k-step horizon, shown as open circles), and a densely sampled \(K=96,T=0.75\) collapse trajectory; 35 runs in total. Among the new configurations, one (\(K=48,T=0.75\)) collapsed terminally, and two (\(K=32\), \(T=0.60\) and \(0.65\)) underwent large excursions that later recovered; the latter appear as high-burst-rate survivors between the calm survivors and the collapses in the dose--response panel.
...
\begin{figure}[h]\centering
\includegraphics[width=.97\textwidth]{figures/sdn_dose_by_block_pi.png}
\caption{Per-block dose--response: each panel plots the run-level pre-onset
burst rate against that block's exact \(\Pisurr\) at step 2000. The
monotone relation holds in every block; blocks 0--4 separate the eventual
deaths most sharply, while blocks 6--7 carry a higher baseline gain in all
runs and overlap more.}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=.97\textwidth]{figures/sdn_dose_by_block_approx.png}
\caption{The same per-block dose--response using the approximation
\(b^2/(4T\delta)\).}
\end{figure}
