...
only ``post-collapse'' points are measured after a collapse.}
\end{figure}
...
the earliest healthy worst-block value. Currently the eighteen fixed-\(T\)
campaign runs with checkpointed ladders are included (\(K = 32, 64\)); the
running boundary-bracketing campaign (\(K = 32\)--\(128\), sixteen
configurations) will extend it.
