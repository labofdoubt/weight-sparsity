...
training; two dose--response panels plot the pre-onset burst rate against
the worst-block value at a common reference step 2000 (thousands of steps
before the earliest observed collapse) and at initialization (step 0,
before any gradient update). The step-0 values come from one untrained
forward pass: at fixed seed the initial weights are identical across
configurations, so the initialization tensor is shared and only
\((K, J, T)\) vary.
...
only.
\item \textbf{Initialization-time screening.} The worst-block \(\Pisurr\)
of a candidate \((K, J, T)\) is computable from one untrained forward
pass, and in the fate maps this step-0 value already separates the eventual
deaths (all observed deaths have worst-block \(\Pisurr(0) \gtrsim 250\)),
with a dose--response as clean as the step-2000 one. A configuration can
therefore be screened before spending any training compute. Being
configuration-level, the step-0 value carries no within-run temporal
signal; \(\neff\) remains the temporal early warning.
\end{itemize}
