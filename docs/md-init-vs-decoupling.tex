...
\author{}
\date{2026-09-21. Final: all 42 runs finished.}
...
\caption{Validation CE for the basic LapSum family, split into panel
columns ($K{=}32$, $T{=}1$), ($K{=}128$, $T{=}1$), ($K{=}128$, $T{=}2$).
Within a panel, color encodes $J$ and line style encodes the regime (full MD
solid, \code{md\_init} $wd{=}0.1$ dashed, $wd{=}0.01$ dotted). The second
row repeats each panel with the vertical axis clipped at 1.8 nats. A cross
marks the point at which a run was stopped as diverged; crosses at a panel's
top edge belong to curves that left its plotted range.}
\end{figure}
...
panels as in the previous figure. In the middle panel every run of both
$(K,J)$ pairs diverges within the first $\sim$2000 steps.}
\end{figure}
...
$(128,128)$ & 2 & 1.4778 & 1.4867 & \dv{6460} \\
$(128,384)$ & 1 & \dv{1880} & \dv{1800} & \dv{1640} \\
$(128,384)$ & 2 & 1.4602 & 1.4671 & 1.4874 \\
\bottomrule
\end{tabular}
\end{center}
...
gradient); color encodes $K$; the lower panel clips the vertical axis at
1.8 nats. No run in this family diverged.}
\end{figure}
