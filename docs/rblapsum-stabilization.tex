...
normalization and support scale}
\author{}
\date{2026-09-21/22.}
...
support $0.3$      & 1 & $6\times10^{-4}$ & \code{rblapsum\_support\_scale}$=0.3$ \\
\bottomrule
\end{tabular}
\end{center}

The first four arms ran on one 8-GPU machine (24 runs, names
\code{ca\_rout\_rblapsum\_kappa\_*\_stab\_*}); the support-scale arm ran on
...
exempt. The support scale multiplies the boundary-exchange term $g_s$
...
\toprule
$(K,J)$ & baseline & $T=2$ & lr $2\times10^{-4}$ & post-norm & support $0.3$ \\
\midrule
...
\includegraphics[width=.82\textwidth]{figures/stab_k32.png}
\caption{$K=32$: every arm survives, so this row measures cost rather than
stabilization. The baseline and $T=2$ curves are indistinguishable; the
output RMSNorm wins at $J=32$ but loses $0.047$ nats at $J=480$, and the
reduced learning rate is clearly behind throughout. Colour encodes the
arm, line style the candidate window ($J=K$ dashed, $K+J=512$ solid).}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=.82\textwidth]{figures/stab_k64.png}
\caption{$K=64$: still no divergence, but the interventions begin to
separate. The output RMSNorm is best at both $J$, by $0.012$--$0.045$ nats
over the baseline.}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=.82\textwidth]{figures/stab_k128.png}
\caption{$K=128$: the baseline (blue) diverges at both $J$ and leaves the
plotted range, its stop marked by a cross at the top edge. All four
interventions converge; the output RMSNorm (red) runs lowest throughout,
ahead of $T=2$, then support scale $0.3$, then the reduced learning rate.}
\end{figure}
...
The support-scale result is worth stating explicitly, because it is the
...
last -- are the robust part. Only one support-scale value ($0.3$) and one
reduced learning rate ($2\times10^{-4}$) were tested, so neither arm is
tuned; a milder support scale in particular might retain the stabilization
