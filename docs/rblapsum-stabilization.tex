...
right, with the vertical axis capped at 2.0 nats, which is where the arms
differ. Colour encodes the arm and line style its kind: the untouched
...
slightly transparent. Every arm survives here, so this configuration
measures cost rather than stabilization: the baseline and $T=2$ curves are
indistinguishable, the output RMSNorm wins at $J=32$ but loses $0.047$ nats
at $J=480$, and the reduced learning rate is clearly behind throughout.}
\end{figure}
...
by $0.012$--$0.045$ nats over the baseline, and the ordering of the four
surviving arms is legible over the second half of training.}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{figures/stab_k128.png}
\caption{$K=128$, panels as above. The baseline (blue) diverges at both
$J$ and leaves the plotted range almost immediately, its stop marked by a
cross at the top edge of each panel. All four interventions converge, and
the ordering is the same at both $J$ and stable from about step 3000: the
output RMSNorm (red) runs lowest throughout, ahead of $T=2$, then the
support-gradient scale, then the reduced learning rate.}
\end{figure}
