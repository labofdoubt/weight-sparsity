...
black) with every surrogate cell sharing that pool (solid, light to dark with
increasing $J$). Left column $T=2$, right column post-norm; the lower row
repeats both panels with the vertical axis capped at 2.0 nats.
...
trained at that $K$ (solid, light to dark with increasing $J$) against all
five hard Top-$\Kp$ baselines (dashed, a separate ramp, light to dark with
increasing $\Kp$). The hard curves are identical in all four figures, so they
serve as a fixed reference scale.
